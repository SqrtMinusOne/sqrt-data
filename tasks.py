...
def waka_task():
    p = subprocess.run(['sqrt_data', 'waka', 'get_data'])
    if p.returncode != 0:
        return
...
def mpd_task():
    p = subprocess.run(['sqrt_data', 'mpd', 'load_library'])
    if p.returncode != 0:
        return
    subprocess.run(['sqrt_data', 'mpd', 'load_logs'])
...
def aw_task():
    ...
    if p.returncode != 0:
        return
    suprocess.run('sqrt_data', 'aw', 'preprocessing_dispatch')
