...
#+end_src
* Install
There are multiple ways to install the project.

** Python
To install the package with Python, run:
...
*** Usage
To make an development environment, run:
#+begin_src bash :eval no
...
Have to evaluate the variable with the definition for the =-f= flag.
#+begin_src scheme :tangle (my/org-prj-dir "sqrt-data.scm")
sqrt-data
#+end_src
** Client side
...
Run =sqrt_data cron run-client-cron= in background.
** Server deployement
*** Docker
