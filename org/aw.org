...
The postprocessing logic turned out to be rather complex... But well.
...
After some time, I've decided to add the invervals of /being/ AFK to this table as well, but with title & app equal to =AFK=. So first, the function to check the AFK status:
#+begin_src sql
...
#+end_src
** Final flow
Thw flow that executes all other flows.
