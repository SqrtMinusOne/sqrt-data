...
As of now, there are 4 default desktop event types with the following structure of the =data= fields, which are quite self-descriptive:
- =afkstatus=
...
#+begin_src python :tangle (my/org-prj-dir "sqrt_data/models/aw/android_unlock.py")
import sqlalchemy as sa
from .bucket import Bucket
...
#+begin_src python :tangle (my/org-prj-dir "sqrt_data/models/aw/android_currentwindow.py")
import sqlalchemy as sa
from .bucket import Bucket
...
Buckets have a lot of data, so we need to somehow save the point of the last download. That seems to be fine to persist in a JSON file.
...
#+end_src
** Saving (Android)
I tried to launch desktop scripts on termux, but had issues with compiling numpy. In the end, I decided that it will be easier to manually save files from the mobile ActivityWatch and sync them with [[https://play.google.com/store/apps/details?id=dk.tacit.android.foldersync.lite&hl=en&gl=US][FolderSync]].
** Loading (Dekstop)
:PROPERTIES:
...
** Loading (Android)
:PROPERTIES:
:header-args:python: :tangle (my/org-prj-dir "sqrt_data/parse/aw/load_android.py") :comments link
:END:
...
A function to get dataframes from the JSON file:
#+begin_src python
def get_dataframes(h):
...
And a function to put everything to the database:
#+begin_src python
TABLE_NAMES = {
...
And because there is a lot of data, some pre-aggregation is necessary to avoid loading the database in live mode. I've considered implementing this part in Python but decided that PL/pgSQL would be easier.
...
I've reimplemented this part a few times, and the most elegant way seems to be doing a join on the =overlaps= operator. CTEs are meant to increase the performance because otherwise doing such a join on tables with around a million records is quite expensive.
#+begin_src sql
...
Finally, we have to put all of that into the table. Previously, I had a materialized view that was refreshed every 24 hours, but it took 15-30 minutes to do the refresh precisely because the join is expensive.
...
This turned out to be much faster, and even the full processing with that approach started to take just 20-30 seconds, so persisting whether the day was processed is not quite necessary. But I'll let it be like this as long it works.
